...
export default function VoluntariadosCarousel({ voluntariados }: Props) {
  ...
  const items = useMemo(() => voluntariados.slice(0, 5), [voluntariados]);
  const slideCount = items.length;

  useEffect(() => {
    if (!slideCount || paused) return;
    const id = setTimeout(() => setCurrentSlide((p) => (p + 1) % slideCount), 5000);
    return () => clearTimeout(id);
  }, [currentSlide, slideCount, paused]);

  // 1 card por slide; el track avanza de a 1

  const next = () => {
    setPaused(true);
    setCurrentSlide((p) => (slideCount ? (p + 1) % slideCount : 0));
    setTimeout(() => setPaused(false), 600);
  };
  const prev = () => {
    setPaused(true);
    setCurrentSlide((p) => (slideCount ? (p - 1 + slideCount) % slideCount : 0));
    setTimeout(() => setPaused(false), 600);
  };
    ...
    <Box sx={{ py: 6, px: { xs: 2, md: 8 } }}>
      ...
      <Box sx={{ position: 'relative' }}
        ...
        <Box sx={{ overflow: 'hidden', borderRadius: 3 }}>
          <Box sx={{ display: 'flex', transition: 'transform 0.5s ease', transform: `translateX(-${currentSlide * 100}%)`, width: `${Math.max(1, slideCount) * 100}%` }}>
            {items.map((v, idx) => (
              <Box key={v.id ?? idx} sx={{ flex: '0 0 100%' }}>
                <Card
                  sx={{
                    display: 'flex',
                  ...
                  <CardContent sx={{ p: 0, minWidth: 0 }}>
                    ...
                    <Typography
                      ...
                      sx={{
                        ...
                        display: '-webkit-box',
                        WebkitLineClamp: { xs: 4, md: 3 } as any,
